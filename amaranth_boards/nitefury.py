...
class _BasePlatform(XilinxPlatform):
    ...
    def toolchain_program(self, product, name, *, programmer="vivado", flash=True):
        assert programmer in ("vivado", "openocd", "openfpgaloader")
        ...
        if programmer == "vivado":
            ...
            # Place the bscan_spi_xc7a100t.bit or bscan_spi_xc7a200t proxy bitstream under a directory
            # named "proxy" in one of the above directories so OpenOCD finds it.
            if flash:
                with product.extract("{}.bin".format(name)) as fn:
                    subprocess.check_call([openocd, "-f", "board/nitefury.cfg",
                        "-c", """init;
                        jtagspi_init 0 [find proxy/bscan_spi_{}.bit];
                        jtagspi_program {} 0;
                        xc7_program xc7.tap;
                        shutdown""".format(self.device, fn)])
            else:
            ...
            with product.extract("{}.bin".format(name)) as fn:
                subprocess.check_call([openfpgaloader, "-b", "acornCle215", fn])
